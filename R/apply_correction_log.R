...
if (any(correction_log_ready_ps$tool == tool_name)) {
  ...
  clean_data.tool2$School_Operationality <- raw_data.tool2$School_Operationality |> apply_log(log = correction_log_ready_ps |> filter(tool == tool_name & Tab_Name == "School_Operationality"))
  clean_data.tool2$Shifts_Detail <- raw_data.tool2$Shifts_Detail |> apply_log(log = correction_log_ready_ps |> filter(tool == tool_name & Tab_Name == "Shifts_Detail"))
  clean_data.tool2$Headmasters <- raw_data.tool2$Headmasters |> apply_log(log = correction_log_ready_ps |> filter(tool == tool_name & Tab_Name == "Headmasters"))
...
correction_log_discrep <- rbind(
  ...
  compare_dt(clean_data.tool2$School_Operationality, raw_data.tool2$School_Operationality) |>
    mutate(tool = "Tool 2 - Light", Tab_Name = "School_Operationality"),

  compare_dt(clean_data.tool2$Shifts_Detail, raw_data.tool2$Shifts_Detail) |>
    mutate(tool = "Tool 2 - Light", Tab_Name = "Shifts_Detail"),

  compare_dt(clean_data.tool2$Headmasters, raw_data.tool2$Headmasters) |>
    mutate(tool = "Tool 2 - Light", Tab_Name = "Headmasters"),
