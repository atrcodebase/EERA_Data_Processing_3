...
for (sheet in names(clean_data.tool0_for_client)[-1]) {
  clean_data.tool0_for_client[[sheet]] <- clean_data.tool0_for_client[[sheet]] |> select(-any_of(c(extra_cols.tool0, qa_cols, "starttime"))) |>
...
for (sheet in names(clean_data.tool2_for_client)[-1]){
  clean_data.tool2_for_client[[sheet]] <- clean_data.tool2_for_client[[sheet]] |> select(-any_of(c(extra_cols.tool2, qa_cols, "starttime"))) |>
    select(Site_Visit_ID, EMIS_School_ID_CBE_KEY, everything())
}
...
for (sheet in names(clean_data.tool3_for_client)[-1]){
  clean_data.tool3_for_client[[sheet]] <- clean_data.tool3_for_client[[sheet]] |> select(-any_of(c(extra_cols.tool3, qa_cols, "starttime"))) |>
    select(Site_Visit_ID, EMIS_School_ID_CBE_KEY, everything())
}
...
for (sheet in names(clean_data.tool4_for_client)[-1]){
  clean_data.tool4_for_client[[sheet]] <- clean_data.tool4_for_client[[sheet]] |> select(-any_of(c(extra_cols.tool4, qa_cols, "starttime"))) |>
    select(Site_Visit_ID, EMIS_School_ID_CBE_KEY, everything())
}
...
for (sheet in names(clean_data.tool5_for_client)[-1]){
  clean_data.tool5_for_client[[sheet]] <- clean_data.tool5_for_client[[sheet]] |> select(-any_of(c(extra_cols.tool5, qa_cols, "starttime"))) |>
    select(Site_Visit_ID, EMIS_School_ID_CBE_KEY, everything())
}
...
for (sheet in names(clean_data.tool6_for_client)[-1]){
  clean_data.tool6_for_client[[sheet]] <- clean_data.tool6_for_client[[sheet]] |> select(-any_of(c(extra_cols.tool6, qa_cols, "starttime"))) |>
    select(Site_Visit_ID, EMIS_School_ID_CBE_KEY, everything())
}
...
for (sheet in names(clean_data.tool7_for_client)[-1]){
  clean_data.tool7_for_client[[sheet]] <- clean_data.tool7_for_client[[sheet]] |> select(-any_of(c(extra_cols.tool7, qa_cols, "starttime"))) |>
    select(Site_Visit_ID, EMIS_School_ID_CBE_KEY, everything())
}
...
for (sheet in names(clean_data.tool8_for_client)[-1]){
  clean_data.tool8_for_client[[sheet]] <- clean_data.tool8_for_client[[sheet]] |> select(-any_of(c(extra_cols.tool8, qa_cols, "starttime"))) |>
    select(Site_Visit_ID, EMIS_School_ID_CBE_KEY, everything())
}
