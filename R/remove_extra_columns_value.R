...
clean_data.tool1_for_client_ulabeled$School_Operationality_Other_... <- clean_data.tool1_ulabeled$School_Operationality_Other_... %>% select(-any_of(c(extra_cols.tool1, qa_cols,"School_CBE_Gender_Based_On_The_Sample", "starttime"))) |>
  left_join(clean_data.tool1_for_client_ulabeled$data |> select(KEY, EMIS_School_ID_CBE_KEY), by = c("PARENT_KEY" = "KEY")) |> 
  select(Site_Visit_ID, EMIS_School_ID_CBE_KEY, everything())
...
clean_data.tool1_for_client_ulabeled$Other_Shifts_Detail <- clean_data.tool1_ulabeled$Other_Shifts_Detail %>% select(-any_of(c(extra_cols.tool1, qa_cols, "starttime"))) |>
  left_join(clean_data.tool1_for_client_ulabeled$data |> select(KEY, EMIS_School_ID_CBE_KEY), by = c("PARENT_KEY" = "KEY")) |> 
  select(Site_Visit_ID, EMIS_School_ID_CBE_KEY, everything())
...
clean_data.tool1_for_client_ulabeled$Weekly_Schedule_Old <- clean_data.tool1_ulabeled$Weekly_Schedule_Old %>% select(-any_of(c(extra_cols.tool1, qa_cols, "starttime"))) |>
  left_join(clean_data.tool1_for_client_ulabeled$data |> select(KEY, EMIS_School_ID_CBE_KEY), by = c("PARENT_KEY" = "KEY")) |> 
  select(Site_Visit_ID, EMIS_School_ID_CBE_KEY, everything())

clean_data.tool1_for_client_ulabeled$Weekly_Schedule_New <- clean_data.tool1_ulabeled$Weekly_Schedule_New %>% select(-any_of(c(extra_cols.tool1, qa_cols, "starttime"))) |>
  left_join(clean_data.tool1_for_client_ulabeled$data |> select(KEY, EMIS_School_ID_CBE_KEY), by = c("PARENT_KEY" = "KEY")) |> 
  select(Site_Visit_ID, EMIS_School_ID_CBE_KEY, everything())

clean_data.tool1_for_client_ulabeled$Additional_Subjects <- clean_data.tool1_ulabeled$Additional_Subjects %>% select(-any_of(c(extra_cols.tool1, qa_cols, "starttime"))) |>
  left_join(clean_data.tool1_for_client_ulabeled$data |> select(KEY, EMIS_School_ID_CBE_KEY), by = c("PARENT_KEY" = "KEY")) |> 
  select(Site_Visit_ID, EMIS_School_ID_CBE_KEY, everything())
...
clean_data.tool7_for_client_ulabeled$C6_list_members <- clean_data.tool7_ulabeled$C6_list_members %>% select(-any_of(c(extra_cols.tool7, qa_cols, "starttime"))) |>
